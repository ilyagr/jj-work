...
fn get_repo_root(workspace_root: &PathBuf) -> eyre::Result<PathBuf> {
...
impl Environment {
    pub fn new(sh: &Shell, config: Settings) -> eyre::Result<Self> {
        // TODO: Non-UTF-8?
    ...
    fn repo_shell(&self) -> eyre::Result<Shell> {
        let sh = Shell::new()?;
    ...
    pub fn workspace_shell(&self, name: &str) -> eyre::Result<Shell> {
        let sh = Shell::new()?;
        sh.change_dir(self.path(name));
        Ok(sh)
    }

    pub fn create_workspace(&mut self, name: &str, revisions: &[String]) -> eyre::Result<()> {
    ...
    pub fn delete_workspace(&mut self, name: &str) -> eyre::Result<()> {
    ...
    pub fn is_valid_workspace(&self, name: &str) -> eyre::Result<bool> {
    ...
    pub fn list_workspaces(&self) -> eyre::Result<Vec<String>> {
        let sh = self.repo_shell()?;
        Ok(sh
